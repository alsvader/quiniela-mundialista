/**
 * Enlace deep link admin → usuario para avisar el arranque de la fase de
 * eliminatoria y recordar el pronóstico (botón en /admin/usuarios). Presentacional:
 * el link se arma server-side en la página con el próximo partido. Cuando `link` es
 * null se muestra deshabilitado con el motivo en `disabledHint`.
 *
 * Hermano visual de `PaymentReminderButton`: comparten el estilo ghost (outline),
 * el icono de WhatsApp y el verde (tertiary) solo en hover. Se distinguen por label;
 * en la práctica nunca conviven en la misma fila (el de pago aparece para quien NO
 * tiene la participación, este para quien SÍ la tiene activa en eliminatoria).
 */
const base =
...
export function EliminatoriaReminderButton({
  link,
  disabledHint,
}: {
  link: string | null;
  ...
  return (
    <a
      ...
      title="Abrir WhatsApp con el aviso de la fase de eliminatoria"
